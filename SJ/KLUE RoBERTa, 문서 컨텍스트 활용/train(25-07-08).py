...
class Config:
    ...
    # 모델
    model_name = 'klue/roberta-base'  # 'large'는 메모리 많이 사용
        ...
        if context_ids is not None and context_ids.size(1) > 0:
            ...
            for i in range(batch_size):
                ...
                if valid_ctx.any():
                    ...
                    # 어텐션을 통한 컨텍스트 통합
                    para_feat_expanded = para_features[i:i+1].unsqueeze(0)
                    ...
                    # 컨텍스트가 없으면 제로 벡터
                    context_features.append(torch.zeros_like(para_features[i:i+1]))
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_df, train_df['generated'])):
        ...
        print(f"\nModel parameters: {total_params/1e6:.1f}M (trainable: {trainable_params/1e6:.1f}M)")
